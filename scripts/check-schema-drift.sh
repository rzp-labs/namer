...
compare_schemas() {
    ...
    if diff -q "$normalized_baseline" "$normalized_current" > /dev/null 2>&1; then
        ...
        local diff_file="$TEMP_DIR/${service}_drift.diff"
...
generate_report() {
    local output_file="$TEMP_DIR/schema_drift_report.md"
...
EOF
        if [[ ${STASHDB_DRIFT:-0} -ne 0 ]]; then
            cat >> "$output_file" <<EOF
### StashDB Schema Changes

See detailed diff: \`$TEMP_DIR/stashdb_drift.diff\`

EOF
        fi

        if [[ ${TPDB_DRIFT:-0} -ne 0 ]]; then
            cat >> "$output_file" <<EOF
### ThePornDB Schema Changes

See detailed diff: \`$TEMP_DIR/tpdb_drift.diff\`
...
main() {
    ...
    if [[ $STASHDB_DRIFT -ne 0 || $TPDB_DRIFT -ne 0 ]]; then
