...
def test_flagged_wiring_routes_ambiguous(tmp_path, monkeypatch):
    ...
    # File should be in a subdirectory of ambiguous_dir (with unique ID)
    assert out.target_movie_file.parent.parent == ambiguous_dir
    assert out.target_movie_file.parent.name.startswith('video_')

    # Get the actual subdirectory where the file was moved
    ambiguous_subdir = out.target_movie_file.parent

    # Summary should include ambiguous metadata
    summary_path = ambiguous_subdir / f'{out.target_movie_file.stem}_namer_summary.json'
    ...
    note_path = ambiguous_subdir / f'{out.target_movie_file.stem}.ambiguous.json'
    ...
    log_path = ambiguous_subdir / f'{out.target_movie_file.stem}_namer.json.gz'
    try:
