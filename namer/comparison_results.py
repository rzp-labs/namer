...
class LookedUpFileInfo:
    ...
    def as_dict(self, config: NamerConfig):
        ...
        return {
            ...
            'performers': ', '.join([p.name for p in self.performers if (p.role or '').lower() == 'female']) if self.performers else None,
            'all_performers': ', '.join([p.name for p in self.performers]) if self.performers else None,
            'performer-sites': ', '.join([p.alias for p in self.performers if (p.role or '').lower() == 'female' and p.alias]) if self.performers else None,
